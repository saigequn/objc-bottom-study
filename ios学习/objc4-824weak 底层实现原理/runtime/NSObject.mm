...
storeWeak(id *location, objc_object *newObj)
{
    ...
    SideTable *oldTable; // 旧值对象所处的 SideTable
    SideTable *newTable; // 新值对象所处的 SideTable
 ...
 retry:
    ...
    // 清理旧值，如果有旧值，进行 weak_unregister_no_lock操作
    if (haveOld) {
...
{ // *location 指向的对象
 ...
 retry:
    ...
    obj = *location;
    // 如果对象 nil 或者 tagged Pointer 则返回 obj
...
/** 
 * Move a weak pointer from one location to another. 将弱指针从一个位置移动到另一位置。
 * Before the move, the destination must be uninitialized. 在移动之前，destination 必须未初始化。
 * After the move, the source is nil. 移动后，source 置为 nil。
 *
 * This function IS NOT thread-safe with respect to concurrent 
...
#   define POOL_BOUNDARY nil
    ...
    id *autoreleaseNoPage(id obj)
    {
        ...
        // 如果对象是哨兵对象，且没有申请自动释放池内存，则设置一个空占位符存储在tls中，其目的是为了性能
...
public:
    ...
    static inline void *push() 
    {
        ...
        if (slowpath(DebugPoolAllocation)) {
            ...
            // 通过autoreleaseFast压栈一个POOL_BOUNDARY，即压栈哨兵
...
//        }
    ...
    pop(void *token)
    {
        ...
        if (token == (void*)EMPTY_POOL_PLACEHOLDER) {
            ...
            // 如果当前页存在，则将当前页设置为coldPage，token设置为coldPage的初始位置
            page = coldPage();
            token = page->begin();
        } else {
            // 获取token所在页
...
{
    // SideTable散列表并不只是一张，而是很多张，与关联对象表类似
    SideTable& table = SideTables()[this];
    table.unlock();
}
