...
objc_object::rootRetain(bool tryRetain, objc_object::RRVariant variant)
{
    ...
    do {
        ...
        // 判断是否为nonpointer isa
        if (slowpath(!newisa.nonpointer)) {
            // 如果不是nonpointer isa，直接操作散列表sidetable
            ClearExclusive(&isa.bits);
        ...
        uintptr_t carry;
        // 调用addc函数执行extra_rc+1, 即引用计数+1操作，即对bits中的1ULL<<45 (arm64) 即extra_rc，用于该对象存储引用计数值
    ...
    if (variant == RRVariant::Full) {
        if (slowpath(transcribeToSideTable)) {
            ...
            /*
             这么操作的目的在于提高性能，因为如果都存在散列表中，当需要release-1时，需要去访问散列表，每次都需要解锁，比较消耗性能。
             extra_rc存储一半的话，可以直接操作extra_rc即可，不需要操作散列表。性能回提高很多
             */
