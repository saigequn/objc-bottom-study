 ...
 public:
    ...
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            // 通过malloc对容器进行重新分配大小为原来的大小加上新增的大小
            // 根据新的容量大小，开辟一个数组，类型是array_t，通过array()获取
            ...
            // 遍历新的数据移动到容器的起始位置，把新的数据拷贝到容器的起始位置
            ...
            list = addedLists[0]; // 取addedLists的第一个元素，付给list，此时list是一维数组
            validate();
        } 
        else {
            // 1 list -> many lists
            ...
            Ptr<List> oldList = list;   // 获取旧的list
            uint32_t oldCount = oldList ? 1 : 0;
            // 计算容量和 - 旧list个数 + 新的lists的个数
            uint32_t newCount = oldCount + addedCount;
            // 通过malloc对容器进行重新分配大小为原来的大小加上新增的大小
            // 根据新的容量大小，开辟一个数组，类型是array_t，通过array()获取
            ...
            // 判断old是否存在，old存在，将旧的list放入到数组的末尾
...
public:
    ...
    class_rw_ext_t *extAllocIfNeeded() {
        ...
        if (fastpath(v.is<class_rw_ext_t *>())) { // 判断rwe是否存在
            return v.get<class_rw_ext_t *>(&ro_or_rw_ext); // 如果存在则直接获取
        } else {
            // 如果不存在则开辟
