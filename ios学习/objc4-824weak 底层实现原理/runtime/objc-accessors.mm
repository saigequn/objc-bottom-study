...
id objc_getProperty(id self, SEL _cmd, ptrdiff_t offset, BOOL atomic) {
    if (offset == 0) { // 偏移量为0，直接返回本身指针指向的类
        return object_getClass(self);
    }

    // Retain release world
    id *slot = (id*) ((char*)self + offset); //通过对象的指针进行便宜获取
    if (!atomic) return *slot; // 非原子性就直接返回
        
    // Atomic retain release world 原子性就进行加锁处理
...
static inline void reallySetProperty(id self, SEL _cmd, id newValue, ptrdiff_t offset, bool atomic, bool copy, bool mutableCopy)
{
    ...
    if (!atomic) {  // 未加锁
        ...
        *slot = newValue;
        ...
        spinlock_t& slotlock = PropertyLocks[slot]; // spinlock_t加锁处理
        slotlock.lock();
        oldValue = *slot;
        *slot = newValue;        
        slotlock.unlock();
