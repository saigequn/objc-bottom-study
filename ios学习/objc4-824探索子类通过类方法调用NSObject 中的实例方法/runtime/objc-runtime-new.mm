...
addClassTableEntry(Class cls, bool addMeta = true)
{
    ...
    // allocatedClasses 开辟的类的表，在objc_init中的runtime_init初始化创建了
    auto &set = objc::allocatedClasses.get();
    ...
    if (!isKnownClass(cls))
        set.insert(cls);
    if (addMeta)
        addClassTableEntry(cls->ISA(), false); // 添加到allocatedClasses表
}
...
public:
    ...
    // 将分类添加到主类中
    // attachToClass中的外部循环是找到一个分类就会进到attachCategories一次，即找一个就循环一次
    void attachToClass(Class cls, Class previously, int flags)
    {
        ...
        auto &map = get();
        // 找到一个分类进来一次，即一个个加载分类，不要混乱
        auto it = map.find(previously);
        
        // 这里回走进来：当主类没有实现load，分类开始加载，迫使主类加载，会走到if流程里面
        if (it != map.end()) {
...
{
    runtimeLock.assertLocked();
    // 此时只有rw， 需要对rwe进行数据添加，即对rwe 0-1的过程
...
// oldest categories first. 准备分类的数据
// 本类中需要添加属性、方法、协议等，所以需要初始化rwe,
                 ...
                 int flags)
{
    ...
    // 进行rwe的创建: 往本类中添加属性、方法、协议等,即对原来的 clean memory要进行处理了
    ...
    for (uint32_t i = 0; i < cats_count; i++) {
        ...
        if (mlist) {
            if (mcount == ATTACH_BUFSIZ) {
                ...
                rwe->methods.attachLists(mlists, mcount); // 存入mlists的末尾
                mcount = 0;
            }
            // 倒序插入,64的原因是允许容纳64个（最多64个分类）
...
static void methodizeClass(Class cls, Class previously)
{
    ...
    if (list) {
        ...
        if (rwe) rwe->methods.attachLists(&list, 1); // attachLists插入数据
    }
...
static Class realizeClassWithoutSwift(Class cls, Class previously)
{
    ...
    // rw 表示 readWrite，即可读可写，由于其动态性，可能会往类中添加属性、方法、添加协议
...
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    // 调用 +load 方法  Call +load methods (without runtimeLock - re-entrant)
    call_load_methods();
}
...
**********************************************************************/
// readClass的主要作用就是将Mach-O中的类读取到内存，即插入表中，
// 但是目前的类仅有两个信息：地址以及名称，而mach-O的其中的data数据还未读取出来.
Class readClass(Class cls, bool headerIsBundle, bool headerIsPreoptimized)
{
    ...
    if (mangledName != nullptr) {
        ...
        // 因此也不会对ro、rw进行操作（可打断点调试，创建类和系统类都不会进入）
...
**********************************************************************/
// 主要是加载类信息，即类、分类、协议等
// 其实就是做了一件事，将Mach-O文件的section依次读取，并根据内容初始化runtime的内存结构
...
#define EACH_HEADER \
    ...
    // 通过doneOnce一次创建一张表gdb_objc_realized_classes
    if (!doneOnce) {
        doneOnce = YES;
        launchTime = YES;
...
# endif
        ...
        // 通过NXCreateMapTable 创建表，存放类信息，即创建一张类的哈希表, gdb_objc_realized_classes，其目的是为了类查找方便、快捷
    ...
    static size_t UnfixedSelectors;
    {
        ...
        for (EACH_HEADER) {
            ...
            for (i = 0; i < count; i++) {
                ...
                // 注册SEL的操作
                // 调用sel_registerNameNoLock将SEL添加到namedSelectors哈希表中
                SEL sel = sel_registerNameNoLock(name, isBundle);
                if (sels[i] != sel) { // 当sel与sels[i]地址不一致时，需要调整为一致的
                    sels[i] = sel; // selector --> sel并不是简单的字符串，是带地址的字符串
                }
            }
        }
    }
    ...
    for (EACH_HEADER) {
        ...
        for (i = 0; i < count; i++) {
            // 数组中会取出OS_dispatch_queue_concurrent、OS_xpc_object、NSRunloop等系统类，
...
#if SUPPORT_FIXUP
    // Fix up old objc_msgSend_fixup call sites 修复一些消息
    // 修复旧的函数指针调用遗留
    for (EACH_HEADER) {
    ...
    // 发现和处理所有Category
    // 需要在分类初始化并将数据加载到类后才执行，对于运行时出现的分类，将分类的发现推迟到对_dyld_objc_notify_register的调用完成后的第一个load_images调用为止
    ...
    for (EACH_HEADER) {
        // 通过nlclslist中的_getObjc2NonlazyClassList获取Mach-O的静态段__objc_nlclslist非懒加载类表
        classref_t const *classlist = hi->nlclslist(&count);
        for (i = 0; i < count; i++) {
            Class cls = remapClass(classlist[i]);
            // 去打印获取到所有非懒加载类，发现只有实现了+load的类才会被打印
...
{
    if (!cls) return;
    // 查看 RW_REALIZED 是否被标记  （当前 Class 是否初始化一个类的指标）
    ASSERT(cls->isRealized());  // _read_images should realize

    // 查看 RW_LOADED 是否被标记。（记录当前类的 +load 方法是否被被调用。）
...
findMethodInSortedMethodList(SEL key, const method_list_t *list, const getNameFunc &getName)
{
    ...
    for (count = list->count; count != 0; count >>= 1) {
        ...
        if (keyValue == probeValue) {
            ...
            //查看当前probe的前一个和keyvalue是否相等，如果相等继续向前，直到不相等时停止，此时probe还没有执行--所以，当前的probe是list中第一个和keyvalue相等的方法；因为category的方法顺序排在类中的方法前面，这做是导致的结果是当分类中的方法和类中的方法重复时，取出的是分类中的方法而非类中的方法；
...
//    if (!DisableTaggedPointerObfuscation && dyld_program_sdk_at_least(dyld_fall_2018_os_versions)) {
        ...
        // 生成的随机数与 ~_OBJC_TAG_MASK 做 与 运算, 这样是随机的最高位或最低位置为0
...
{
    objc::unattachedCategories.init(32);    // 分类初始化
    objc::allocatedClasses.init();          // 类的表初始化
}
