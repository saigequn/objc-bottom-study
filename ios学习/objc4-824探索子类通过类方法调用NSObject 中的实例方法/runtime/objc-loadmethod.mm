...
static void call_class_loads(void)
{
    ...
    int used = loadable_classes_used; // 调用标记
    loadable_classes = nil;
    loadable_classes_allocated = 0;
    loadable_classes_used = 0;
     
    // Call all +loads for the detached list. 调用列表中的 Class 类的 load 方法
    for (i = 0; i < used; i++) {
        // 获取 Class 指针
    ...
    // Destroy the detached list. 释放 Class 列表
...
void call_load_methods(void)
{
    ...
    do {
        ...
        while (loadable_classes_used > 0) {
            call_class_loads(); // 会从一个待加载的类列表 loadable_classes 中寻找对应的类，并使用 selector(load) 的实现并执行
        }
