...
static inline mask_t cache_hash(SEL sel, mask_t mask) 
{
    ...
    value ^= value >> 7;
#endif
    return (mask_t)(value & mask); //和mask进行一次与运算 mask就是面具
}
...
void bucket_t::set(bucket_t *base, SEL newSel, IMP newImp, Class cls)
{
    ...
    // 原有的imp进行编码（和class进行异或运算）转成 uintptr_t 类型
    uintptr_t newIMP = (impEncoding == Encoded
                        ? encodeImp(base, newImp, newSel, cls)
                        : (uintptr_t)newImp);

    if (atomicity == Atomic) { // 修饰符号Atomic
...
bucket_t *cache_t::allocateBuckets(mask_t newCapacity)
{
    ...
    // 将开辟内存的最后一个位置存入sel = 1，imp = 第一个buket位置的地址
...
void cache_t::reallocate(mask_t oldCapacity, mask_t newCapacity, bool freeOld)
{
    ...
    setBucketsAndMask(newBuckets, newCapacity - 1); //设置mask和buckets的值
...
// 汇编 objc_msgSend  -> objc_msgSend_uncached
// objc_msgSend_uncached --> lookUpImpOrForward --> log_and_fill_cache --> cache_t::insert
void cache_t::insert(SEL sel, IMP imp, id receiver)
{
    ...
    mask_t newOccupied = occupied() + 1; // 表示你是第几个进来缓存的
    unsigned oldCapacity = capacity(), capacity = oldCapacity;
    if (slowpath(isConstantEmptyCache())) {
        // Cache is read-only. Replace it.
        if (!capacity) capacity = INIT_CACHE_SIZE; // 只有第一次缓存方法的时，才会去开辟容量默认开辟容量是 capacity = INIT_CACHE_SIZE 即capacity = 4 就是4个bucket的内存大小
        ...
        reallocate(oldCapacity, capacity, true); // 扩容的时候会进行一步重要的操作，开辟新的内存，释放回收旧的内存，此时的freeOld = true
    }

    // 首先拿到bucket()指向开辟这块内存首地址，也就是第一个bucket的地址，bucket()既不是数组也不是链表，只是一块连续的内存
    bucket_t *b = buckets();
    mask_t m = capacity - 1;
    mask_t begin = cache_hash(sel, m); // hash函数根据缓存sel和mask，计算出hash下标。
    // 为什么需要mask呢？mask的实际作用是告诉系统你只能存前capacity - 1中的位置，比如capacity = 4时，缓存的方法只能存前面3个空位
    mask_t i = begin;
    ...
    do {
        if (fastpath(b[i].sel() == 0)) {
            incrementOccupied(); // _occupied++ 意思就是我缓存一个 _occupied加1 意思就是占位 bucket的个数等于_occupied
            b[i].set<Atomic, Encoded>(b, sel, imp, cls()); //sel和imp写入bucket，开始缓存方法sel
            return;
        }
        if (b[i].sel() == sel) { // 如果该方法缓存过了，直接return
            // The entry was added to the cache by some other thread
            // before we grabbed the cacheUpdateLock.
            return;
        }
    } while (fastpath((i = cache_next(i, m)) != begin)); // 如果存在hash冲突，hash冲突就是方法sel不一样，下标一样，此时会进行再次hash，冲突解决继续缓存
