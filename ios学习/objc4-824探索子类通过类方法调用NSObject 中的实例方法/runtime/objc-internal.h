...
_objc_decodeTaggedPointer_noPermute(const void * _Nullable ptr)
{
    ...
    if ((value & _OBJC_TAG_NO_OBFUSCATION_MASK) == _OBJC_TAG_NO_OBFUSCATION_MASK)
        return value;
#endif
    // 与混淆器 objc_debug_taggedpointer_obfuscator 进行异或操作
...
_objc_makeTaggedPointer(objc_tag_index_t tag, uintptr_t value)
{
    ...
    if (tag <= OBJC_TAG_Last60BitPayload) { // 无扩展标记
        // ASSERT(((value << _OBJC_TAG_PAYLOAD_RSHIFT) >> _OBJC_TAG_PAYLOAD_LSHIFT) == value);
        // tagged point主要分为三部分：tag mask、tag index、payload。我们先分别将它们的值通过位移计算 移动到正确的位置，然后再进行或运算将它们组合到一起
        uintptr_t result =
            (_OBJC_TAG_MASK | 
...
_objc_getTaggedPointerTag(const void * _Nullable ptr) 
{
    ...
    uintptr_t value = _objc_decodeTaggedPointer(ptr); // 先将tagged pointer解码
    // 再通过位移操作，获取basicTag 和 extTag
    uintptr_t basicTag = (value >> _OBJC_TAG_INDEX_SHIFT) & _OBJC_TAG_INDEX_MASK;
...
_objc_getTaggedPointerValue(const void * _Nullable ptr) 
{
    // 首先 Tagged Pointer 解码，与 objc_debug_taggedpointer_obfuscator 进行异或操作，然后根据不同平台的宏定义进行移位操作。
    // ASSERT(_objc_isTaggedPointer(ptr));
    uintptr_t value = _objc_decodeTaggedPointer_noPermute(ptr);
    uintptr_t basicTag = (value >> _OBJC_TAG_INDEX_SHIFT) & _OBJC_TAG_INDEX_MASK;
    if (basicTag == _OBJC_TAG_INDEX_MASK) {
...
_objc_getTaggedPointerSignedValue(const void * _Nullable ptr) 
{
    ...
    uintptr_t value = _objc_decodeTaggedPointer_noPermute(ptr);
    uintptr_t basicTag = (value >> _OBJC_TAG_INDEX_SHIFT) & _OBJC_TAG_INDEX_MASK;
    if (basicTag == _OBJC_TAG_INDEX_MASK) { // 判断是否有扩展标记
        return ((intptr_t)value << _OBJC_TAG_EXT_PAYLOAD_LSHIFT) >> _OBJC_TAG_EXT_PAYLOAD_RSHIFT; // 通过相应的位移操作，得到value
    } else {
        return ((intptr_t)value << _OBJC_TAG_PAYLOAD_LSHIFT) >> _OBJC_TAG_PAYLOAD_RSHIFT; // 通过相应的位移操作，得到value
    }
}
