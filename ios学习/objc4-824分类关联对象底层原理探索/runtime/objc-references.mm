...
public:
    ...
    inline void acquireValue() {
        if (_value) { //判断value是否存在
            switch (_policy & 0xFF) { // 除了retain、copy外的策略，不需要处理
            case OBJC_ASSOCIATION_SETTER_RETAIN:    // 策略为retain
                _value = objc_retain(_value);
                break;
            case OBJC_ASSOCIATION_SETTER_COPY:      // 策略为copy
