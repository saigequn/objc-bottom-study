...
public:
  ...
  // Inserts key,value pair into the map if the key isn't already in the map.如果key-value不在map中，则插入key-value
  // The value is constructed in-place if the key is not in the map, otherwise
  // it is not moved.如果key不在map中，则直接构造，否则它不会移动
  template <typename... Ts>
  std::pair<iterator, bool> try_emplace(const KeyT &Key, Ts &&... Args) {
    BucketT *TheBucket;
    if (LookupBucketFor(Key, TheBucket)) // LookupBucketFor找桶子，其中Key为关联对象
      return std::make_pair(
               makeIterator(TheBucket, getBucketsEnd(), true),
               false); // Already in map.已经存在，直接返回--false

    // Otherwise, insert the new element.
    TheBucket = InsertIntoBucket(TheBucket, Key, std::forward<Ts>(Args)...);//往桶里加值
    return std::make_pair(
             makeIterator(TheBucket, getBucketsEnd(), true),
             true);//bool值为true，表示第一次往哈希map添加桶子
  }
