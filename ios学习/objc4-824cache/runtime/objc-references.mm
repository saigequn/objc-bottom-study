...
public:
    ...
    AssociationsHashMap &get() {
        return _mapStorage.get(); // 从静态变量中取出来，所以全场唯一
    }
...
_object_set_associative_reference(id object, const void *key, id value, uintptr_t policy)
{
    ...
    // 局部作用域空间
    {
        ...
        // manager.get() -> _mapStorage.get() 通过_mapStorage.get()生成哈希map，其中_mapStorage是一个静态变量，所以 哈希map 永远是通过静态变量获取出来的，所以是全场唯一的
        
        if (value) {
            auto refs_result = associations.try_emplace(disguised, ObjectAssociationMap{});
            if (refs_result.second) { // 判断第二个存不存在，即bool值是否为true
                /* it's the first association we make 第一次建立关联 */
                isFirstAssociation = true; // nonpointerIsa ，标记位true
            }
